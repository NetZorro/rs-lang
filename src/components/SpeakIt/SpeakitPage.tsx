...
//    const wordsList = await WordServices.getWordList(0, 20); // - получаем IWord из вне, а дальше делаем все что надо
    ...
    const xRoundWordArray = WordServices.getRoundWordsArray(
      wordsListWithSuccess,
      10
    );

    changeGameState({
      type: "NEWROUND",
      payload: { ...gameState, roundWordArray: xRoundWordArray },
    });
  };
  ...
  useEffect(() => {
    ...
    if (word !== undefined) {
      ...
      changeGameState({
        type: "ADDWORD",
        payload: { ...gameState, roundWordArray: arr },
      });
  ...
  useEffect(() => {
    if (restart) {
      ...
      changeGameState({
        type: "RESULT",
        payload: { ...gameState, result: false },
      });
      changeGameState({
        type: "MICROPHONE",
        payload: { ...gameState, microphoneOn: false },
      });
      setRestart(false);
    }
  }, [restart]);
  ...
  function Card(word: IWordWithSuccess, idx: number) {
    ...
    return (
      <div
        className={cl("cards__item", {
          activeCard:
            selectWord.id === word.id ||
            (gameState.roundWordArray[gameState.roundWordArray.indexOf(word)]
              .success &&
              gameState.microphoneOn),
        })}
        data-wordid={word.id}
        onClick={(event) => {
          registerCardsClickEvent(word);
        }}
      >
        <span className="cards__item--audio-icon" data-wordid={word.id}>
          <img data-wordid={word.id} src={audio} alt="audio icon" />{" "}
        </span>
        <p className="cards__item--word" data-wordid={word.id}>
          {word.word}
        </p>
        <p className="cards__item--transcription" data-wordid={word.id}>
          {word.transcription}
        </p>
      </div>
    );
  ...
  function Star() {
    return <div className="info__score--star" />;
  }

  function ResultWord(word: IWordWithSuccess) {
    return (
      <div
        className="resultpage__item"
        data-wordid={word.id}
        onClick={(event) => {
          registerResultWordClickEvent(word.audio);
        }}
      >
        <span className="resultpage__item--audio-icon">
          <img data-wordid={word.id} src={audio} alt="audio icon" />
        </span>
        <p className="resultpage__item--word" data-wordid={word.id}>
          {word.word}
        </p>
        <p className="resultpage__item--transcription" data-wordid={word.id}>
          {word.wordTranslate}
        </p>
        <p className="resultpage__item--translation" data-wordid={word.id}>
          {word.transcription}
        </p>
      </div>
    );
  ...
  function registerResultWordClickEvent(wordAudio: string) {
    playAudio(
      "audio",
      `https://raw.githubusercontent.com/irinainina/rslang/rslang-data/data/${wordAudio}`
    );
  }
    ...
    <div className="main-speakit mt-0" id="fsArea">
        ...
        <div className="startpage d-flex align-items-center">
          <div className="container px-5 text-center">
            ...
            <p className="startpage--intro-text">
              The SpeakIt game helps you improve your english pronunciation. The
              more words you say correctly, the more experience points you'll
            ...
            <a
              ...
              onClick={() =>
                changeGameState({
                  type: "INPROGRESS",
                  payload: { ...gameState, inProgress: true },
                })
              }
            >
              Start
      ...
      {gameState.inProgress && !gameState.result && (
        <div className="speakit-container flex-column justify-content-center align-items-center py-4">
          <div className="speakit-wrapper d-flex flex-column justify-content-center align-items-center container">
            <div className="speakit-info d-flex flex-column flex-md-row align-items-center justify-content-md-between">
              <div className="speakit-info__pages">
                <span
                  className="speakit-info__pages--page activePage"
                  data-groupno="0"
                />
              ...
              <div className="speakit-info__score pt-3 pt-md-0">
                {gameState.roundWordArray.map(
                  (item: IWordWithSuccess, idx: number) => {
                    if (item.success) {
                      return Star();
                    }
                  }
            ...
            <div className="current d-flex flex-column align-items-center">
              {selectWord.image ? (
                <img
                  ...
                  src={templatesURL.getImageURL(selectWord.image)}
              ...
              <p className="current__translation text-center">
                {selectWord.wordTranslate}
              </p>
              <input
                type="text"
                value={message}
                className={cl("current__input", {
            ...
            <div className="btns d-flex flex-column flex-md-row mt-4">
              <a
                href="#"
                className="btn btn-lg btn-info btns__restart mb-2 mb-md-0 mx-0 mx-md-2"
                onClick={() => {
                  setRestart(true);
                }}
              >
              ...
              <a
                href="#"
                className="btn btn-lg btn-info btns__speak mb-2 mb-md-0 mx-0 mx-md-2"
                onClick={() =>
                  changeGameState({
                    type: "MICROPHONE",
                    payload: {
                      ...gameState,
                      microphoneOn: !gameState.microphoneOn,
                    },
                  })
                }
              >
              ...
              <a
                href="#"
                className="btn btn-lg btn-info btns__result mb-2 mb-md-0 mx-0 mx-md-2"
                onClick={() =>
                  changeGameState({
                    type: "RESULT",
                    payload: { ...gameState, result: true },
      ...
      {gameState.result && (
        <div className="resultpage align-items-center justify-content-center">
          <div className="resultpage__container">
            <p className="results__container--errors">
              Errors
              <span className="resultpage__errors-num">
                {WordServices.getCountError(gameState.roundWordArray)}
              </span>
            </p>
            ...
            <p className="results__container--success">
              Success
              <span className="resultpage__success-num">
                {WordServices.getCountSuccess(gameState.roundWordArray)}
              </span>
            </p>
            ...
            <div className="resultpage__btns-res d-flex flex-column flex-sm-row justify-content-center text-center mt-4">
              <a
                href="#"
                className="btn btn-info btn-md resultpage__return mb-2 m-sm-0 mx-0 mx-sm-3"
                onClick={() =>
                  changeGameState({
                    type: "RESULT",
                    payload: { ...gameState, result: false },
              ...
              <a
                href="#"
                className="btn btn-info btn-md resultpage__new-game mb-2 m-sm-0 mx-0 mx-sm-3"
